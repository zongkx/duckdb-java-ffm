...
public class DuckdbJdbcResultSet implements java.sql.ResultSet {
    ...
    private DuckDBResultSetMetaData metaData;
    ...
    @Override
    public String getString(String cLabel) throws SQLException {
        return getString(findColumn(cLabel));
    }

    @Override
    public int getInt(String cLabel) throws SQLException {
        return getInt(findColumn(cLabel));
    }

    @Override
    public long getLong(String cLabel) throws SQLException {
        return getLong(findColumn(cLabel));
    }

    @Override
    public float getFloat(String cLabel) throws SQLException {
        return getFloat(findColumn(cLabel));
    }

    @Override
    public double getDouble(String cLabel) throws SQLException {
        return getDouble(findColumn(cLabel));
    }

    @Override
    public boolean getBoolean(String cLabel) throws SQLException {
        return getBoolean(findColumn(cLabel));
    }

    @Override
    public byte getByte(String cLabel) throws SQLException {
        return getByte(findColumn(cLabel));
    }

    @Override
    public short getShort(String cLabel) throws SQLException {
        return getShort(findColumn(cLabel));
    }
    ...
    public ResultSetMetaData getMetaData() throws SQLException {
        checkOpen();
        if (metaData == null) {
            int colCount = (int) nativeResult.getTotalColumns();
            String[] names = new String[colCount];
            String[] typeNames = new String[colCount];
            for (int i = 0; i < colCount; i++) {
                names[i] = nativeResult.getColumnName(i);
                try {
                    typeNames[i] = nativeResult.getColumnTypeName(i);
                } catch (Exception e) {
                    typeNames[i] = "VARCHAR";   // 安全的回退，保证元数据不抛异常
                }
            }
            metaData = new DuckDBResultSetMetaData(names, typeNames);
        }
        return metaData;
    }
    ...
    @Override
    public Date getDate(String cLabel) throws SQLException {
        throw new SQLFeatureNotSupportedException();
    }

    @Override
    public Time getTime(String cLabel) throws SQLException {
        throw new SQLFeatureNotSupportedException();
    }

    @Override
    public Timestamp getTimestamp(String cLabel) throws SQLException {
        throw new SQLFeatureNotSupportedException();
    }
